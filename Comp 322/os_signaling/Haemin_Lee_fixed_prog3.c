...
/*
operator = ->
dereference the memory address and the accessing the struct member
example
...
void cancel_worker(thread_info_t *info)
{
	...
	/*https://unix.stackexchange.com/questions/403988/how-to-kill-an-individual-thread-under-a-process-in-linux*/
...
static void *scheduler_run(void *unused)
{
	...
	time_val.it_interval.tv_sec = (long)QUANTUM;
	time_val.it_interval.tv_nsec = (long)0;
	time_val.it_value = time_val.it_interval;

	if(timer_settime(timer, 0, &time_val, NULL) == -1){
		perror("can not start timer");
		exit(-1);
	}
